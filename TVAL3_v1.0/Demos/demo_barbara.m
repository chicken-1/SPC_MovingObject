...
t = cputime - t;
% calculate relative error (by frobenius norm: root of sum of the absolute squares of its elements 
re_er = norm(estIm-Im,'fro')/norm(Im,'fro'); 

b2 = A(estIm(:),1);
bavg = mean(abs(b));
[estIm2, out] = TVAL3(A,b2,sidelength,sidelength,opts);
...
xlabel(sprintf('Rel-Err: %4.2f%%,   CPU time: %4.2fs',100*re_er,t),'fontsize',14);
